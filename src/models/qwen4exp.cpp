...
ggml_tensor * llama_model_qwen4exp::graph::build_qsa_top_k(
    ...
    if (inp == nullptr) {
        ...
        // blk_bias consumes the per-block half of the bias (added to the block-level score),
        // so it must be [n_blocks]; the non-blk_bias path consumes it per-cell ([n_kv])
    ...
    if (inp->dirty_rows != nullptr) {
        ...
        // count blocks along ne1: rms_norm launches gridDim.y = ne2, capped at 65535
