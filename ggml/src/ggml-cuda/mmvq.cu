...
template <ggml_type type, bool has_fusion = false, bool has_clamp = false, ggml_glu_op active_glu = GGML_GLU_OP_SWIGLU>
...
// ============================================================================
// Fusion-generic multi-token MoE mat-vec (standard path). Ported from upstream
// master to support #27621 (MoE GLU/topk-router fusion extended to speculative
// decoding, which drives the draft model's MoE as a multi-token batch with gate
// fusion). Distinct name keeps it separate from the SwiGLU-hardcoded dual-gate
// moe-cache kernel (mul_mat_vec_q_moe) used by ggml_cuda_moe_cache_mmv_fused.
// ============================================================================
template <ggml_type type, int c_rows_per_block, bool has_fusion = false>
__launch_bounds__(get_mmvq_mmid_max_batch_for_device<type>()*ggml_cuda_get_physical_warp_size(), 1)
static __global__ void mul_mat_vec_q_moe_std(
...
template <ggml_type type>
static void mul_mat_vec_q_moe_std_launch(
        const void * vx, const void * vy, const int32_t * ids, const ggml_cuda_mm_fusion_args_device fusion, float * dst,
        const uint32_t ncols_x, const uint3 nchannels_y, const uint32_t nrows_x,
        const uint32_t stride_row_x, const uint32_t stride_col_y, const uint32_t stride_col_dst,
        const uint32_t stride_channel_x, const uint32_t stride_channel_y, const uint32_t stride_channel_dst,
    ...
    if (has_fusion) {
        ggml_cuda_kernel_launch(mul_mat_vec_q_moe_std<type, rows_per_block, true>, launch_params,
            vx, vy, ids, fusion, dst, ncols_x, nchannels_y, nrows_x,
            stride_row_x, stride_col_y, stride_col_dst,
            stride_channel_x, stride_channel_y, stride_channel_dst,
            ncols_dst, ids_stride);
    } else {
        ggml_cuda_kernel_launch(mul_mat_vec_q_moe_std<type, rows_per_block, false>, launch_params,
            vx, vy, ids, fusion, dst, ncols_x, nchannels_y, nrows_x,
            stride_row_x, stride_col_y, stride_col_dst,
            stride_channel_x, stride_channel_y, stride_channel_dst,
            ncols_dst, ids_stride);
...
static void mul_mat_vec_q_switch_ncols_dst(
    ...
    if (has_ids && ncols_dst > 1) {
        ...
        // Multi-token MUL_MAT_ID with optional gate/bias fusion (draft-model MoE under
        // specdec, #27621). Uses the fusion-generic _std kernel, NOT the SwiGLU moe-cache
        // kernel, so fusion args are honoured for arbitrary token counts.
        mul_mat_vec_q_moe_std_launch<type>(
...
void ggml_cuda_mul_mat_vec_q(
    ...
    if (fusion) {
        // Multi-token MUL_MAT_ID + fusion is supported via the _std MoE kernel (#27621),
        // so the old single-token restriction is lifted.
